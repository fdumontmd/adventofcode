...
pub fn process(input: &'static str) -> Result<String, AocError> {
    ...
    for card in &cards {
        let current_card_count = counts[card.id - 1];
        let wins = card.wins();
        for count in counts.iter_mut().skip(card.id).take(wins) {
            *count += current_card_count;
        }
    }
    Ok(format!("{}", counts.into_iter().sum::<usize>()))
}
...
";
    ...
    #[rstest]
    #[case(INPUT, "30")]
    #[case(include_str!("../input.txt"), "5659035")]
